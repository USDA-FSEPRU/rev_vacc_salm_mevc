...
proteomes_epitope_homology_serovar <- foreach(Serovar=unique(proteomes_epitope_homology$Serotype_wgs), .combine = rbind) %do% {
  ...
  full_0_epitope_percentage <- sum(subset_serovar$V6=="TRUE")/nrow(subset_serovar)*100
  Unique_Id = subset_serovar$V1
  data.frame(Serovar, full_0_epitope_percentage) #Bind all elements above to data frame
}
...
write_tsv(proteomes_epitope_homology_serovar, paste0("epitope_blasting_results_summaries_Step_3/", Unique_Id, "_proteomes_epitope_homology_serovar_summary.tsv"))
