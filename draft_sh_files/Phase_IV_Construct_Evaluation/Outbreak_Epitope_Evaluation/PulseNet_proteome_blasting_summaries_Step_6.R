...
#Check for number of complete homology hits for each isosrc
proteomes_epitope_homology_isosrc <- foreach(IsoSrc=unique(proteomes_epitope_homology$Summarized.isolation.source), .combine = rbind) %do% {
  ...
  full_0_epitope_count<- sum(subset_isosrc$V6=="TRUE")
  data.frame(IsoSrc, full_0_epitope_count) #Bind all elements above to data frame
}
...
write_tsv(proteomes_epitope_homology_isosrc, paste0("epitope_blasting_results_summaries_Step_6/", Unique_Id, "_proteomes_epitope_homology_isosrc_counts.tsv"))
