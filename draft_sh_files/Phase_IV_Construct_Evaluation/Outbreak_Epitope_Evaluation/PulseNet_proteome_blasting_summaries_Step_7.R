...
proteomes_epitope_homology_otbksrc <- foreach(OtbkSrc=unique(proteomes_epitope_homology$Summarized.outbreak.source), .combine = rbind) %do% {
  ...
  full_0_epitope_percentage <- sum(subset_otbksrc$V6=="TRUE")/nrow(subset_otbksrc)*100
  data.frame(OtbkSrc, full_0_epitope_percentage) #Bind all elements above to data frame
}
...
write_tsv(proteomes_epitope_homology_otbksrc, paste0("epitope_blasting_results_summaries_Step_7/", Unique_Id, "_proteomes_epitope_homology_otbksrc_summary.tsv"))
